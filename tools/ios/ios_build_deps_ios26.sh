...
compile_ios_deps() {
	...
	target_sysroot="$(xcrun --sdk ${sdk} --show-sdk-path)"
	if [ ! -d "$target_sysroot" ]; then
		echo "Requested target sysroot SDK not found for ${arch}."
		exit 1
	fi
	...
	if [ ! -d "$host_sysroot" ]; then
		echo "Requested host sysroot SDK not found for macOS."
		exit 1
	fi
	...
	export CC="$(xcrun --sdk ${sdk} --find clang) -isysroot $target_sysroot"
	#export CC="clang -arch arm64"
	#export CFLAGS="-arch arm64"
	export CXX="$(xcrun --sdk ${sdk} --find clang++) -isysroot $target_sysroot"
	...
	if [ ! -f "$check_file" ] || [[ "$step" == *"gmp"* ]]; then
		...
		#./configure "--prefix=$dir/install" --with-pic M4=/usr/local/Cellar/m4/1.4.20/bin/m4
		CC_FOR_BUILD="$HOST_CC" \
		./configure "--prefix=$dir/install" --with-pic M4=/opt/homebrew/Cellar/m4/1.4.20/bin/m4 \
